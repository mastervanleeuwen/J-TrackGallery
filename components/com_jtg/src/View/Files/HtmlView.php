...
namespace Jtg\Component\Jtg\Site\View\Files;
...
class HtmlView extends JtgView
{
	...
	function _displayList($tpl)
	{
		...
		$action = Route::_('index.php?option=com_jtg&view=files&layout=list', false);
	...
	function _displayUserTracks($tpl)
	{
		...
		$action = Route::_('index.php?option=com_jtg&view=files&layout=user', false);

		$lists['order'] = $filter_order;
		$lists['order_Dir'] = $filter_order_Dir;

		$cats = JtgModel::getCatsData(true);
		$sortedter = JtgModel::getTerrainData(true);
		$params = $mainframe->getParams();
		$this->params = $params;
		$this->sortedter = $sortedter;
