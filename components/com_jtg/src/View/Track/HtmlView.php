...
namespace Jtg\Component\Jtg\Site\View\Track;
...
class HtmlView extends JtgView
{
	...
	public function display($tpl = null)
	{
		...
		// folder
		$app->getLanguage()->load('com_jtg_additional', JPATH_SITE . '/images/jtrackgallery', 'en-GB',true);
		$app->getLanguage()->load('com_jtg_additional', JPATH_SITE . '/images/jtrackgallery', null, true);
		...
		if (isset ($this->id))
		{
			...
			if (!$gpsData->displayErrors())
			{
				...
				$this->mapJS = JtgMapHelper::parseTrackMapJS($gpsData,$this->track->id, $this->track->default_map, $this->imageList, $makepreview, true, ComponentHelper::getParams('com_jtg')->get('jtg_param_show_layerswitcher'),$colors,'jtg_map');
				...
				if (version_compare(JVERSION,'4.0','ge')) {
					$this->tags = $model->getTable('Files','Administrator')->getTagsHelper()->getItemTags('com_jtg.file' , $this->id);
					$this->tagids = $model->getTable('Files','Administrator')->getTagsHelper()->getTagIds($this->id, 'com_jtg.file');
				}
				else {
