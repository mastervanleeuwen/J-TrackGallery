...
class TrackModel extends FormModel
{
	...
	function saveFile ()
	{
		...
		$trackTable = $this->getTable('Files');
		...
		if ((int) $params->get('upload_notify_uid'))
		{
			...
			if (strlen($recipient))
			{
				...
				$link = Route::_(Uri::base() . "index.php?option=com_jtg&view=track&layout=default&id=" . $rows->id);
	...
	function deleteFile ($id)
	{
		...
		// Delete from DB
		if (!$this->getTable('Files')->delete($id))
		{
			return false;
		}
	...
	function download($id, $format, $track)
	{
		...
		switch ($format)
		{
			case "kml":
				...
				$file .= "<description>Generated by " . $app->getCfg('sitename') . " " . Uri::base() . "</description>";
				$file .= "<Folder>";
				$file .= "<Placemark>";
				$file .= "<name>" . $track->title . "</name>";
			...
			case "gpx":
				...
				$metadata .= "<copyright author=\"" . $app->getCfg('sitename') . "\" />\n";
				$metadata .= "<link href=\"" . Uri::base() . "index.php?option=com_jtg&amp;view=track&amp;id=" . $id . "\">\n";
				$metadata .= "<text>" . $track->title . " on " . $app->getCfg('sitename') . "</text>";
				$metadata .= "</link>\n";
				...
				$trk .= "<link href=\"" . Uri::base() . "index.php?option=com_jtg&amp;view=track&amp;id=" . $id . "\" />\n";
				$trk .= "<trkseg>\n";
				$minlat = 180;
				$maxlat = -180;
				$minlon = 90;
			...
			case "tcx":
				...
				$file .= "      <Name>" . $app->getCfg('sitename') . "</Name>\n";
				$file .= "      <Build>\n";
				$file .= "        <Version>\n";
...
<?php
	...
	function savecomment ($id, $cfg)
	{
		...
		if ($cfg->inform_autor == 1)
		{
			$mailer = Factory::getMailer();
			$config = Factory::getConfig();
			$link = Route::_(Uri::base() . "index.php?option=com_jtg&view=track&layout=default&id=" . $id);
	...
	function parseHomepageIcon ($www)
	{
		...
		$cfg = JtgHelper::getConfig();
		$return = "<a target=\"_blank\" href=\"" . $www . "\"><img src=\"" . Uri::base() . "components/com_jtg/assets/template/" . $cfg->template .
